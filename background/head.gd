...
func rotate_to_wanted(wanted_direct, delta):
	if !insta_turn:
		var angle = Vector2(current_direction.z, current_direction.y).angle_to(Vector2(wanted_direct.z, wanted_direct.y))
		if angle > 0.1:
			...
			current_direction = wanted_direct
			turn_speed = 0.01
	else:
		current_direction = wanted_direct

func _physics_process(delta: float) -> void:
	if Input.is_action_just_pressed("worm_dash"):
		start_turn()
	if Input.is_action_just_pressed("patrol_inner"):
		state = Globals.STATE.INNER_PATROL
		radius_y = 140
		radius_z = 250
	if Input.is_action_just_pressed("patrol_outer"):
		state = Globals.STATE.OUTER_PATROL
		radius_y = 300
		radius_z = 500
	if Input.is_action_just_pressed("follow_player"):
		Globals.speed = 150
...
func start_dash():
	...
	tween.tween_method(change_speed, 150, 600, 0.5)
	$scourge_head6.bite()
	$DashTimer.start()
...
func _on_dash_timer_timeout() -> void:
	...
	turn_speed = 0.01
	var tween = create_tween()
	tween.tween_method(change_speed, 500, 150, 0.5)
